...
def test_ansi_editor_ctrl_z_undo(console):
    """#532: Ctrl+Z undoes the last ANSI cell change."""
    _open_editor(console, 'ANSI EDIT "A:/tests/ANSI_UNDO.ANS"')
    _keys(console, b"\x1b[C\x1b[C\x1b[B\x1b[B")
    _keys(console, bytes([1]) + b"1")
    _keys(console, b" ")
    assert _is_colour(console.screen_pixel(20, 40), (170, 0, 0)), console.screen_pixel(20, 40)
    _keys(console, bytes([26]))  # Ctrl+Z
    assert max(console.screen_pixel(20, 40)) < 40, console.screen_pixel(20, 40)
    _quit_editor(console)
