...
static const char kHelpHelp[] =
	...
	"the Index; Escape again quits. One press is enough.\n"
...
static const char kHelpEdit[] =
	...
	"colours follow the theme. Non-Turbo themes use\n"
	"custom palettes. Saved as edit_theme in\n"
	".mmbasic.ini (or OPTION EDIT THEME name).\n"
	"Keys: F2 save  F3 open  F4 #include  F9 run\n"
	"      ^O save  Alt+X quit\n"
...
static const char kHelpWordpad[] =
	...
	"\n"
	"Markdown: # / ## headings (larger type), -/* bullets,\n"
	"> quotes, ``` fenced code blocks, **bold** / __bold__.\n"
...
static const char kHelpTerm[] =
	...
	"uses MODE 16 (1920x1080). Connects to host:port as\n"
	"soon as TERM starts (up to about 25s for DNS+TCP).\n"
	"Failures say Network not available, DNS failed,\n"
	"TCP timeout, or TCP refused. A second TERM after a\n"
	"timeout does not wait on a leftover SYN. Drawing\n"
	"uses TEXT on PAGE 1, copied to PAGE 0; scrolling is\n"
	"a PAGE pixel shift. ANSI/VT100 (SGR colours, cursor,\n"
	"ED/EL) plus telnet TTYPE=ANSI and DA/DSR replies for\n"
	"BBS autodetection. Glyphs are IBM VGA 8x16 (CP437),\n"
	"so box drawing and shade characters 176-178 match a\n"
	"DOS VGA terminal. F10 or Alt+X exits and restores the\n"
	"previous MODE. Alt+F opens a File menu (Exit). Esc is\n"
	"sent to the host; press Esc twice or wait briefly to\n"
	"flush a lone Esc. USB Backspace (DEL) is sent as BS.\n"
	"\n"
	"Telnet IAC handling matches CONNECT. Line mode with\n"
	"local echo until the server negotiates otherwise.\n"
	"In character mode (SGA) Enter sends CR only.\n"
	"\n"
