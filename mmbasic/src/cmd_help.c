...
static const char kHelpTerm[] =
	"TERM host$, port\n"
	"\n"
	"Fullscreen telnet-style terminal. Default is an\n"
	"80-column pane centred on a dark slate background\n"
	"with cream text (boxed). File-menu Boxed/Full toggles\n"
	"the MODE 14 width (960x540, 120 columns) with no\n"
	"letterbox. The choice is for this TERM session only.\n"
	"Connects to host:port as soon as TERM starts (up to\n"
	"about 25s for DNS+TCP). Failures say Network not\n"
	"available, DNS failed, TCP timeout, or TCP refused.\n"
	"A second TERM after a timeout does not wait on a\n"
	"leftover SYN. Incoming TCP is drained before a pane\n"
	"redraw so a busy BBS does not starve receive. HDMI\n"
	...
	"NAWS reports 80 columns when boxed and 120 when full.\n"
	...
	"Alt+F opens a File menu (Exit, Echo ON/OFF, Boxed/Full,\n"
	"Bookmarks). Bookmarks persist in C:/.termconfig (A: if\n"
