...
static const char kHelpWordpad[] =
	...
	"h1/h2/h3 same theme colour at 32x64 / 24x48 / 16x32\n"
	"native glyphs, not a scaled-up 8x16 body face),\n"
...
static const char kHelpTerm[] =
	...
	"text. Uses MODE 14 (960x540). Connects to host:port as\n"
	...
	"timeout does not wait on a leftover SYN. Incoming TCP\n"
	"is drained before a pane redraw so a busy BBS does not\n"
	"starve receive. HDMI present is the 80-column pane,\n"
	"not the full framebuffer. Drawing uses TEXT on PAGE 1,\n"
	...
	"Alt+F opens a File menu (Exit, Echo ON/OFF). Esc is\n"
	...
	"local echo until the server sends IAC WILL ECHO.\n"
	"Hosts that never negotiate (many MUDs) keep local\n"
	"echo so typed text is visible. Toggle Echo from the\n"
	"File menu when a host echoes anyway or hides input.\n"
