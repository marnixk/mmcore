...
static const char kIndexCommands[] =
	...
	"  SETTICK FACTORY_RESET OPTIONS CONNECT TERM IPCONFIG ETHERNET CREDITS CONTINUE EXIT LS\n"
	"\n"
	"Prompt\n"
...
static const char kHelpColour[] =
	...
	"Colours are RGB integers: RGB(r,g,b), RGB(r,g,b,t),\n"
	"RGB(RED), or RGB(\"RED\"). t is 0 (clear) to 15 (solid)\n"
	"when drawing on page 1, the CMM2 overlay.\n"
	"Integers 0 to 31 are the IBM PC\n"
...
static const char kHelpPage[] =
	...
	"page n. COPY copies src to dst. Flag B skips black\n"
	"pixels (CMM2 blit). Page 1 is composited over the\n"
	"display page: black is clear, RGB(...,t) blends.\n"
...
static const char kHelpWordpad[] =
	...
	"Each heading letter advances by its own ink width\n"
	"so spacing is proportional, wrap still follows the\n"
	"column width, and the cursor block fits the glyph.\n"
...
static const char kHelpTerm[] =
	"TERM [host$, port]\n"
	"\n"
	"Fullscreen telnet-style terminal. Menu and status\n"
	"colours follow the OPTION EDIT THEME palette (same\n"
	"as the EDITOR). The terminal pane and letterbox\n"
	"background are always black. With no arguments TERM\n"
	"starts disconnected; use Alt+T and Bookmarks to\n"
	"connect from a saved favourite or add a new one.\n"
	"With host$ and port it connects at once.\n"
	"Default is an 80-column pane centred on a black\n"
