...
`TDF.BAS` loads the TheDraw `.TDF` fonts in `A:/fonts/tdf/` and draws banner
text in TEXT mode. It is an include file:
...
s$` draws at character cell (column, row). The library uses file stream #10
until `TDF.Close`; see `HELP TDF`.
