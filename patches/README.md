...
- Two bulk-IN buffers (ping-pong). A data completion immediately arms
  the other slot, and `SendFrame` posts IN before the blocking OUT.
  `SetCompleteOnNAK` stays so QEMU idle NAKs do not freeze the host.
